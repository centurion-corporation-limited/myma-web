...
class MerchantController extends Controller
{
    ...
    public function getDashboard(Request $request)
    {
        ...
        try{
            ...
            if(!$auth_user->hasRole('restaurant-owner-catering')){
              ...
              foreach($orders as $order){
                  ...
                  if(isset($s_orders[$batch_id])){
                      ...
                      $s_orders[$batch_id]['address'] = $address;
                      $s_orders[$batch_id]['id'] = $order->id;
                      $s_orders[$batch_id]['item_count'] = $count;
              ...
              //   $noww->addDay();
              // }
              $subs = Subscription::where('delivery_date', $noww->toDateString())->get();
              $listing = [];
              $today = Carbon::now();
              $b = $l = $d = 0;
              $brk_arr = $lch_arr = $din_arr = [];
              foreach($subs as $sub){
                if($sub->item){
                    if($sub->item->restaurant_id != $restaurant_id){
                      continue;
                    }
                }else{
                  continue;
                ...
                $order = $sub->order;
                if($order->dormitory_id){
                  $batch = Batch::where('dormitory_id', $order->dormitory_id)->whereDate('batch_date', $today->toDateString())->first();
                  if(!$batch){
                    ...
                    $dd['batch_l'] = 'BL'.$num;
                    $dd['batch_d'] = 'BD'.$num;
                    $dd['batch_date'] = $today->toDateString();
                    $batch = Batch::create($dd);
                  }
                  $address = @$batch->dormitory->name;
                }
                else{
                  $batch = Batch::where('address','like', $order->address)->whereDate('batch_date', $today->toDateString())->first();
                  if(!$batch){
                    $batch = Batch::orderBy('created_at', 'decs')->orderBy('id', 'decs')->first();
                    $dd['address'] = $order->address;
                    $num = str_pad($batch->id+1, 7, '0', STR_PAD_LEFT);
                ...
                $order_item_db = OrderItem::where('order_id', $sub->order_id)->where('item_id', $sub->item_id)->first();

                if($sub->b_allowed){
                  $status_id = $sub->b_status == ""?8:$sub->b_status;
                  $brk_arr[$b]['item_count'] = 1;//$order_item_db?$order_item_db->quantity:1;
                  $brk_arr[$b]['id'] = $sub->order_id;
                  if($sub->breakfast){
                    $brk_arr[$b]['batch_id'] = $sub->breakfast;
                  }else{
                    $sub->update(['breakfast' => $batch->batch_b]);
                    $brk_arr[$b]['batch_id'] = $batch->batch_b;
                  }
                  $brk_arr[$b]['address'] = $address;
                  $brk_arr[$b]['breakfast'] = true;
                  $brk_arr[$b]['status_id'] = $status_id;
                  $brk_arr[$b]['pickup_time'] = Carbon::now()->format('d/m/Y 07:00:00');
                  $b++;
                }
                if($sub->l_allowed){
                  $status_id = $sub->l_status == ""?8:$sub->l_status;
                  $lch_arr[$l]['id'] = $sub->order_id;
                  $lch_arr[$l]['item_count'] = 1;//$order_item_db?$order_item_db->quantity:1;
                  if($sub->lunch){
                    $lch_arr[$l]['batch_id'] = $sub->lunch;
                  }else{
                    $sub->update(['lunch' => $batch->batch_l]);
                    $lch_arr[$l]['batch_id'] = $batch->batch_l;
                  }
                  $lch_arr[$l]['address'] = $address;
                  $lch_arr[$l]['lunch'] = true;
                  $lch_arr[$l]['status_id'] = $status_id;
                  $lch_arr[$l]['pickup_time'] = Carbon::now()->format('d/m/Y 12:00:00');
                  $l++;
                }
                if($sub->d_allowed){
                  $status_id = $sub->d_status == ""?8:$sub->d_status;
                  $din_arr[$d]['id'] = $sub->order_id;
                  $din_arr[$d]['item_count'] = 1;//$order_item_db?$order_item_db->quantity:1;
                  if($sub->dinner){
                    $din_arr[$d]['batch_id'] = $sub->dinner;
                  }else{
                    $sub->update(['dinner' => $batch->batch_d]);
                    $din_arr[$d]['batch_id'] = $batch->batch_d;
                  }
                  $din_arr[$d]['address'] = $address;
                  $din_arr[$d]['dinner'] = true;
                  $din_arr[$d]['status_id'] = $status_id;
                  $din_arr[$d]['pickup_time'] = Carbon::now()->format('d/m/Y 19:00:00');
                  $d++;
                }
              }
              $listing = array_merge($brk_arr, $lch_arr, $din_arr);
              $list_orders = [];
              if(count($listing)){
                  foreach($listing as $lst){
                    if(isset($list_orders[$lst['batch_id']])){
                        $list_orders[$lst['batch_id']]['id'] .= ','.$lst['id'];
                        $list_orders[$lst['batch_id']]['order_count'] += 1;
                        $list_orders[$lst['batch_id']]['item_count'] += $lst['item_count'];

                        if($list_orders[$lst['batch_id']]['order_count'] == $list_orders[$lst['batch_id']]['picked_count']){
                          //$list_orders[$lst['batch_id']]['status_id'] = $list_orders[$lst['batch_id']]['status_id'];
                        }else{
                          //$list_orders[$lst['batch_id']]['status_id'] = 9;
                        }
                    }else{
                        $list_orders[$lst['batch_id']] = $lst;
                        $list_orders[$lst['batch_id']]['picked_count'] = 0;
                        if($lst['status_id'] == 10){
                          $list_orders[$lst['batch_id']]['picked_count'] = 1;
                        }
                        $list_orders[$lst['batch_id']]['order_count'] = 1;
                        $list_orders[$lst['batch_id']]['accepted'] = 1;
                    }
                  }
              }
                ...
                if($count){
                  ...
                  if($pass){
                    ...
                    if(isset($b_orders[$batch_id])){
                        ...
                        $b_orders[$batch_id]['address'] = $address;
                        $b_orders[$batch_id]['id'] = $order->id;
                        $b_orders[$batch_id]['item_count'] = $count;
            ...
            // \Log::debug($b_orders);
            $data['orders_p'] = array_values($list_orders);//array_values($b_orders);
            ...
            $data['today_date'] = $dt->format('d/m/Y');//$pickup_time;
    ...
    public function getUpcomingOrder(Request $request)
    {
        ...
        try{
            ...
            if(!$auth_user->hasRole('restaurant-owner-catering')){
              ...
              })->where('accepted', '1')->whereNotIn('status_id', ['10', '11','12','13'])->whereNotNull('delivery_time')->orderBy('created_at', 'asc')->get();
              // /->where('delivery_time', '>=', $time)
              $today = Carbon::now()->addDay()->toDateString();
              foreach($orders as $order){

                  $address = '';
                  \Log::debug($today == $order->delivery_date);
                  if($today == $order->delivery_date){
                    if($order->dormitory_id){
                        $batch = Batch::where('dormitory_id', $order->dormitory_id)->whereDate('batch_date', $order->delivery_date)->first();
                        if(!$batch){
                            $batch = Batch::orderBy('created_at', 'desc')->orderBy('id', 'decs')->first();
                    ...
                    $order->batch_id = '';
                    $address = $order->dormitory?$order->dormitory->name:$order->address;
                  }
              ...
              })->where('delivery_date', $end_date)->orderby('delivery_date', 'asc')->get();
              foreach($subs as $key => $sub){
                  $order = Order::where('id', $sub->order_id)->first();

                  //need to have batch ids for the next day order too
                  $nn = Carbon::now();
                  $del_date = Carbon::parse($sub->delivery_date);
                  $diff_days = $del_date->diffInDays($nn);
                  $batch = "";
                  $address = "";

                  if($order->dormitory_id){
                    $batch = Batch::where('dormitory_id', $order->dormitory_id)->whereDate('batch_date', $del_date->toDateString())->first();
                    if(!$batch){
                              $batch = Batch::orderBy('created_at', 'desc')->orderBy('id', 'desc')->first();
                              $dd['dormitory_id'] = $order->dormitory_id;
                              $num = str_pad($batch->id+1, 7, '0', STR_PAD_LEFT);
                              ...
                              $dd['batch_l'] = 'BL'.$num;
                              $dd['batch_d'] = 'BD'.$num;
                              $dd['batch_date'] = $del_date->toDateString();
                              $batch = Batch::create($dd);
                    }
                    $address = @$batch->dormitory->name;
                  }
                  else{
                    $batch = Batch::where('address','like', '%'.$order->address.'%')->whereDate('batch_date', $del_date->toDateString())->first();
                    if(!$batch){
                              $batch = Batch::orderBy('created_at', 'desc')->orderBy('id', 'desc')->first();
                              $dd['address'] = $order->address;
                              $num = str_pad($batch->id+1, 7, '0', STR_PAD_LEFT);
                  ...
                  //end next day batch id section
                  if($sub->b_allowed){
                      $sub->breakfast = $batch->batch_b;
                      $sub->save();
                      $tt = "07:00:00";
              ...
              foreach($pack_orders as $od){
                if(isset($od['batch_id']) && $od['batch_id'] != ''){
                  if(!in_array($od['batch_id'], $batch_odd)){
                    $batch_odd[] = $od['batch_id'];
                    $b_orders[$od['pickup_time']][$od['batch_id']] = [
                      'id' => $od['id'],
                      'batch_id' => $od['batch_id'],
                      'address' => $od['address'],
                      'item_count' => $od['item_count'],
                      'pickup_time' => $od['pickup_time']
                    ];
                  }else{
                    $b_orders[$od['pickup_time']][$od['batch_id']]['id'] .=  ', '.$od['id'];
                    $b_orders[$od['pickup_time']][$od['batch_id']]['item_count'] +=  $od['item_count'];
                  }
                }else{
              ...
              // $b_odd = array_values($b_orders);
              foreach($b_orders as $b_oo){
                foreach($b_oo as $v_oo){
                  $b_odd[] = $v_oo;
                }
              }
              $p_odd = array_values($p_orders);
              $p_odd = array_merge($b_odd, $p_odd);
              // $orders_p = Order::where('delivery_date', '>=',$now->toDateString())->whereHas('items', function($q) use($restaurant_id){
              //     $q->whereHas('item', function($qq) use($restaurant_id){
              //             $qq->where('restaurant_id', $restaurant_id);
              //     });
    ...
    public function getTwoUpcomingOrder(Request $request)
    {
        $auth_user = JWTAuth::toUser($request->input('token'));
        try{
            $morning_limit = Carbon::createFromTimeString('11:00:00');
            ...
            $data['orders_p'] = $p_odd;
            return response()->json(['status' => 'success', 'data' => $pack_orders, 'message'=> 'DATA'], 200);
    ...
    public function viewOrder(Request $request)
    {
        ...
        try{
            ...
            foreach($items as $item){
                ...
                $response[$item->item_id]['start_date'] = Carbon::parse($item->start_date)->format('d/m/Y');
                $response[$item->item_id]['end_date'] = Carbon::parse($item->end_date)->format('d/m/Y');
        ...
        try{
            ...
            if($auth_user->hasRole('restaurant-owner-single')){
              ...
              foreach($orders as $order){
                ...
                // }
                if($order->created_at){
                  $order->ordered_at = Carbon::parse($order->created_at)->format('d/m/Y H:i A');
                }else{
                  $order->ordered_at = $order->created_at;//->format('d/m/Y H:i A');
                }
    ...
    public function packageSubscribers(Request $request)
    {
        try{
            ...
            foreach($orders as $order){
                ...
                $order->subscription_start = Carbon::parse($order->delivery_date)->format('d/m/Y');
                $subs = Subscription::where('order_id', $order->id)->orderBy('delivery_date', 'desc')->first();
                if($subs){
                  $order->subscription_end = Carbon::parse($subs->delivery_date)->format('d/m/Y');
                }else{
        ...
        try{
            ...
            if($package){
                ...
                foreach($items as $item){
                  ...
                  if($sub){
                      if($sub->breakfast == $batch_id){
                        $sub->update(['b_status' => 9]);  
                      }elseif($sub->lunch == $batch_id){
                        $sub->update(['l_status' => 9]);  
                      }elseif($sub->dinner == $batch_id){
                        $sub->update(['d_status' => 9]);
                      }else{
                        $sub->update([$label => 9]);
                      }
        ...
        try{
            ...
            foreach($orders as $order){
              ...
              foreach($order->items as $itm){
                if($itm->item){
                  $sub_total += $itm->item->base_price*$itm->quantity;
                }
              }
              $order->sub_total = $order->total;//$total;
              //($trans != '' && $trans->food_share != '')?$trans->food_share:
              $dd[$batch_id]['batch_id'] = $batch_id;
              $dd[$batch_id]['sub_total'] = isset($dd[$batch_id]['sub_total'])?($dd[$batch_id]['sub_total']+$sub_total):$sub_total;
    ...
    public function getAccountDetail(Request $request)
    {
        try{
            ...
            if($package){
              ...
              ->select('order_items.*', 'food_menu.base_price as price','food_menu.name')
              ->get();
    ...
    public function getInvoiceDetail(Request $request)
    {
        try{
            ...
            $invoice->invoice_date = Carbon::parse($order->delivery_date)->format('d/m/Y');
            $invoice->discount = 0;
            $invoice->id = $batch_id;
