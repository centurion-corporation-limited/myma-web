...
class MenuController extends Controller
{
        ...
        try{
            ...
            foreach($data as $item){
                ...
                elseif($item->slug == 'free-wifi' && $item->icon == '')
                    $item->icon = static_file('images/icon-mom.png');
                elseif($item->slug == 'invoices' && $item->icon == '')
                    $item->icon = static_file('images/icon-mom.png');
    ...
    public function getListNew(Request $request)
    {
        try{
            ...
            // $category = MenuCategory::all()->toArray();
            // $menus = $category;

            $data = Menu::select('id','name','name_bn','name_ta','name_mn','name_th', 'access','slug', 'icon', 'category_id', 'url', 'type')->where('active', '1')->whereNotIn('id', $not_allowed);

            if(!empty($user) && $user->type == 'free'){
                $data->where('access', $user->type);
            }

            $data = $data->orderBy('category_id', 'asc')->get();
            $menus = [];
            foreach($data as $item){
                if($label != ''){
                    if($item['name_'.$label] != ''){
                ...
                elseif($item->slug == 'food')
                    $item->url = url('').'/customer/dashboard?token='.$token.'&time='.time();
                elseif($item->slug == 'invoices')
                    $item->url = route('customer.invoices.list', $token);
                elseif($item->slug == 'games')
                    $item->url = getOption('games_link');//'http://gamecenter.yogrt.co/g/v1/#/?appid=152324230061613002';
                elseif($item->slug == 'free-wifi')
                    $item->url = getOption('wifi_link');
                ...
                elseif($item->slug == 'free-wifi' && $item->icon == '')
                    $item->icon = static_file('images/icon-mom.png');
                elseif(($item->slug == 'topup' || $item->slug == 'invoices') && $item->icon == '')
                    $item->icon = static_file('images/icon-mom.png');
                elseif($item->slug == 'food' && $item->icon == '')
                    $item->icon = static_file('images/naan.png');
                else{
                ...
                if(isset($menus[$item->category->name])){
                    $menus[$item->category->name][] = $item;
                }else{
                    $menus[$item->category->name] = [];
                    $menus[$item->category->name][] = $item;
                }
                //$menus[$item->category_id][] = $item;
            }
            return response()->json(['status' => 'success', 'data' => $menus, 'message' => 'SUCCESS'], 200);
