...
class MenuController extends Controller
{
    ...
    {
        $data = $request->only('name', 'order', 'name_ta','name_bn','name_mn', 'name_th', 'access', 'category_id', 'type');
    ...
    public function postEdit($id, EditMenuRequest $request)
    {
        ...
        $data = $request->only('name', 'active', 'order', 'name_ta','name_bn','name_mn', 'name_th', 'access', 'category_id', 'type');
        // $data['slug'] = str_slug($data['name']);
        if($data['order'] == ''){
            $data['order'] = Menu::get()->count()+1;
        }
