...
use App\Http\Requests\EditCourseShareRequest;
...
class ShareController extends Controller
{
    ...
    public function postShare($merchant_id, ShareRequest $request)
    {
        /** @var User $item */
        $auth_user = \Auth::user();
        ...
        'merchant_address_2', 'merchant_address_3', 'frequency', 'gst', 'gst_number', 'naanstap_share', 'subscription_limit');
        $item->update($data);
        //add bank account
    ...
    public function updateCourseShare($id, EditCourseShareRequest $request)
    {
        $auth_user = \Auth::user();
        $id = decrypt($id);
        ...
        $data = $request->only('merchant_share', 'myma_transaction_share', 'product_type', 'merchant_address_1', 'merchant_address_2',
          'merchant_address_3', 'frequency', 'gst', 'gst_number');
    ...
    public function getFoodShare(Request $request)
    {
      ...
      $user_ids = User::whereHas('roles', function($q){
        $q->whereIn('slug', ['restaurant-owner-single', 'restaurant-owner-catering']);//, 'restaurant-owner-catering'
      })->pluck('id');
    ...
    //   }
      if ($user_ids) {
          $items->whereIn('user_id', $user_ids);
      }
      ...
      return view('admin.food.merchant.list', compact('items', 'auth_user','paginate_data', 'limit'));
    }
    ...
    public function updateFoodShare($id, FoodMerchantRequest $request)
    {
        ...
        $naanstap_share = $item->naanstap_share;
        $item->update($data);

        if($naanstap_share != $data['naanstap_share']){
          event(new UpdatePrice($id));
        }
        $bank_name = $request->input('bank_name');
        $acc_number = $request->input('account_number');
        $bank_address = $request->input('bank_address');
