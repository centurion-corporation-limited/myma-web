...
class DebugController extends Controller
{
    ...
    public function getReport(Request $request)
    {
        $auth_user = \Auth::user();
        $items = [];
        $ad_manager = User::whereHas('roles', function($q){
        ...
        if($mid == 'singx'){
          ...
          $data['vendor_sale'] = number_format($item->sum_transaction_amount, 2);//x
          $data['gross'] = number_format($item->sum_myma_share,2);//a
          $data['cost_sharing'] = 0;//b
          $data['txn_fee'] = number_format($item->sum_myma_part,2);//c
          $data['cost_charged'] = number_format($item->sum_singx_part,2);//d
          $data['net'] = $data['gross']+$data['txn_fee']-$data['cost_sharing']-$data['cost_charged'];
          $data['net'] = number_format($data['net'],2);
          $data['payback_vendor'] = $data['vendor_sale']-$data['gross']-$data['txn_fee'];
          $data['payback_vendor'] = number_format($data['payback_vendor'],2);
          ...
          $trans = $trans->selectRaw('*, count(id) as vendor_qty, sum(transaction_amount) as sum_transaction_amount,
          sum(myma_share) as sum_myma_share, sum(myma_part) as sum_myma_part, sum(flexm_part) as sum_flexm_part')
          ->groupBy('mid')->orderBy('created_at', 'asc')->get();

          foreach($trans as $item){
            $product_type = @$item->merchant->product_type;
            if($product_type == ''){
              if(@$item->merchant->user->hasRole('restaurant-owner-catering')){
                $product_type = 'Catering';
              }elseif(@$item->merchant->user->hasRole('restaurant-owner-single')){
            ...
            $data['vendor_name'] = @$item->merchant->merchant_name;
            $data['from_date'] = $from_date;
            $data['to_date'] = $to_date;
            $data['product_type'] = $product_type;
            $data['vendor_qty'] = $item->vendor_qty;
            ...
            $data['cost_sharing'] = 0;//b
            $data['txn_fee'] = number_format($item->sum_myma_part,2);//c
            $data['cost_charged'] = number_format($item->sum_flexm_part,2);//d
            $data['net'] = $data['gross']+$data['txn_fee']-$data['cost_sharing']-$data['cost_charged'];
            $data['net'] = number_format($data['net'],2);
            $data['payback_vendor'] = $data['vendor_sale']-$data['gross']-$data['txn_fee'];
            $data['payback_vendor'] = number_format($data['payback_vendor'],2);
        ...
        $user = \Auth::user();
        $merchants = Merchant::pluck('merchant_name', 'mid');

        return view('admin.revenue.report', compact('items', 'user', 'merchants', 'ad_manager'));
