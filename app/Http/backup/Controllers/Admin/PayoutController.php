...
class PayoutController extends Controller
{
    ...
    public function exportPaymentReport(Request $request)
    {
        ...
        if(count($items)){
            ...
            'Vendor sales quantity', 'Vendor Sales Amount', 'Wallet received amount', 'Revenue earned deducted',
            'Wallet txn fee earn', 'Cost charged where appl', 'Net payable to vendor', 'Action' );
        }

        $amount = $wallet_rec_amt = $revenue_deducted = $txn_fee = $cost_charged = $net_payable = 0;
        foreach($items as $item){
              $arr = [];
    ...
    public function getUsers(Request $request)
    {
      ...
      if($auth_user->hasRole('food-admin')){
        $merchants = FoodMerchant::select('user_id', 'id')->get();
        $items = [];
    ...
    public function getView(Request $request)
    {
        ...
        if ($merchant_id = $request->input('merchant_id')) {
            ...
            if($pending > 1){
              ...
              $total = Payout::where('payout_for', 'wlc')->where('merchant_id', $merchant_id)->where('status', 'pending')->where('payout_date', '<=', $now)->sum('amount');
            }
        ...
        foreach($items as $item){
          ...
          if($merchant){
            ...
            $paymentsArray[] = [
              ...
              $merchant->merchant_name,'',$merchant->merchant_address_1,$merchant->merchant_address_2,$merchant->merchant_address_3,@$merchant->account->account_number,
              '','','','',@$merchant->account->swift_code, @$merchant->account->bank_name, @$merchant->account->bank_address, @$merchant->account->bank_country,
              @$merchant->account->routing_code,'','0', number_format($item->net_payable, 2, '.', ''),'','','','','','','','','','','','','','','','','','','','','','','','','','','','','',''
            ];
            ...
            if(strtolower($item->vendor_code) == 'singx'){
              ...
              $paymentsArray[] = [
                ...
                number_format($item->payback_vendor, 2, '.', ''),'','','','','','','','','','','','','','','','','','','','','','','','','','','','','',''
              ];
            }
          }
        }
    ...
    public function getPaymentReportItems($merchant_id, $from ,$to){
      ...
      $items = Payout::where('payout_for', 'wlc')->whereIn('merchant_id', $merchant_id);
