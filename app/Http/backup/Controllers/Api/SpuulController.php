    ...
    public function cancelSubscription(Request $request){
        ...
        try{
          ...
          if($code == "200" && $reason == "OK"){
            $body = $result->getBody();
            $content = json_decode($body->getContents());
            addActivity('Spuul successfull cancel subscription', @$user_id, @$data, $content);
            return response()->json(['status' => 'success', 'data' => $content, 'message'=> 'DATA'], 200);

          }else{
            addActivity('Spuul cancel subscription reason code', @$user_id, @$data, ['code' => $code, 'reason' => $reason]);
            ...
            addActivity('Spuul error in cancel subscription', @$user_id, @$data, $jsonBody);
            return response()->json(['status' => 'success', 'data' => @$jsonBody->errors, 'message'=> @$msg], 200);
        }catch(GuzzleException $e){
            addActivity('Spuul error in cancel subscription', @$user_id, @$data, $e->getMessage());
