...
<table style="margin: 10px auto; max-width: 90%; width: 100%;">
	...
	<tr>
        ...
        @if($item->merchant->gst)
        <p>GST Number : {{ $item->merchant->gst_number }}</p>
        @endif
			</div>
		</td>
		...
		<td>
			<table style="width: 100%; margin-top: 20px; text-align: right;font-size: 14px;">
			  ...
			  <tr>
			    ...
			    <td style="padding: 5px 10px;">S${{ number_format($item->discount, 2) }}</td>
			  </tr>
			  <tr>
			  	<td style="padding: 5px 10px;"><strong>Naanstap Charges</strong></td>
			   <td style="padding: 5px 10px;">S${{ number_format($item->naanstap, 2) }}</td>
			  </tr>
				<tr>
			  <td style="padding: 5px 10px;"><strong>Transaction Charges</strong></td>
			   <td style="padding: 5px 10px;"><strong>S${{ number_format($item->flexm, 2) }}</strong></td>
       </tr> --}}
			  <tr>
