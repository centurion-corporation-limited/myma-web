  ...
  <div class="login-form">
    ...
    <form id="signupForm" action="{{ route('app.register.new') }}" method="POST" enctype='multipart/form-data'>
      ...
      <i class="fa fa-info-circle"></i>Password should have at least one number (0-9), one lower case (a-z) and one upper case (A-Z) letter, one special character (~!@#$%^&*()+-?) and minimum length of 8.
          ...
          <input type="text" class="form-control icon-fin" required="" pattern="[STFGstfg]{1}[0-9]{7}[a-zA-Z]{1}" title="Fin no format not correct." name="fin_no" placeholder="Fin No*" value="{{ old('fin_no') }}" />
					<span class="help-block">
	          <small></small>
	        </span>
      </div>
      ...
      <div class="form-group dob dum">
        <input type="text" autocomplete="off" name="dob" required value="{{ old('dob') }}" class="form-control icon-date-cal" placeholder="Date of Birth*">
      ...
      <div class="form-group">
        <input type="text" name="street_address" class="form-control icon-street" placeholder="Street Add" value="{{ old('street_address') }}">
      </div>
      ...
      <div class="chk terms dum">
        <label>
          <input type="checkbox" name="terms" requird {{ old('terms') }} @if(old('terms') == 1) checked @endif value="1">
