...
@section('content')
    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            <h2>Add QR Terminal</h2>
            <div class="clearfix"></div>
          </div>
          <div class="x_content">
            <br />
            <form id="demo-form2" action="{{ route('admin.terminal.add') }}" method="POST" enctype="multipart/form-data"  data-parsley-validate class="form-horizontal form-label-left">
              {{ csrf_field() }}

              <div class="form-group">
                <label class="control-label col-md-2 col-sm-2 col-xs-12" for="type">Merchant<span class="required">*</span></label>
                <div class="col-md-6 col-sm-10 col-xs-12">
                  <select class="form-control" name="merchant_id">
                    <option value="">Select a merchant</option>
                    @foreach($merchant as $id => $mer)
                      <option value="{{ $id }}" @if($id == $merchant_id) selected readonly @endif>{{ $mer }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
