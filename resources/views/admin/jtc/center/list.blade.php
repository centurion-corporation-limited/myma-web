...
<div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Main category @if($main) - {{ $main->name }} @endif</h2>
                    {{--@if($auth_user->can('create.mom-category-add'))--}}
                    <ul class="nav navbar-right panel_toolbox">
                      <li><a href="{{ route('admin.jtc.centers.add', ['type' => request('type')]) }}" class="btn btn-success">Add Main Category</a>
                      </li>
                    </ul>
                    {{--@endif--}}
                  ...
                  <div class="x_content x_content_form">
                    <form action="{{ route('admin.jtc.centers.list') }}" method="GET" class="form-inline">
                          ...
                          <a href="{{ route('admin.jtc.centers.list', ['type' => request('type', 'jtc')]) }}" class="btn btn-success">Reset</a>
                    </form>
                    <table id="datatable-responsivee" class="table table-striped table-bordered dt-responsive nowrap table-hover foo_table" cellspacing="0" width="100%">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                            <a href="{{ route('admin.jtc.category.list', ['center_id' => encrypt($item->id), 'type' => request('type')]) }}"><i class="fa fa-2x fa-eye"></i></a>
                            <a href="{{ route('admin.jtc.centers.edit', encrypt($item->id)) }}"><i class="fa fa-2x fa-edit"></i></a>
                            {{--@if($auth_user->can('delete.mom-category-delete'))--}}
                            <a href="{{ route('admin.jtc.centers.delete', ['id' => $item->id, '_token' => csrf_token()]) }}" data-message="Are you sure about deleting this?" class="post-deletee"><i class="fa fa-2x fa-trash-o"></i></a>
                            {{--@endif--}}
                          </td>
                        </tr>
                        @endforeach
