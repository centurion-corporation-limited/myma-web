...
<table style="margin: 10px auto; max-width: 90%; width: 100%;">
	<tr>
		<td colspan="2" style="text-align: center;">
			<h2>
				...
				<img src="{{ static_file('customer/images/iso.jpg') }}" alt="" width="40">
				@else
				<img src="{{ static_file('images/naan.png') }}" alt="" width="40">
				@endif
			    <b>@if($order->type == "package") ISO Delight @else Naanstap @endif</b>
	...
	<tr>
		...
		<td style="text-align: right;">
		    ...
		    @if($order->address)
		      <p>{{ $order->block_no or '' }}</p>
		      <p>{{ $order->address or '' }}</p>
		    @else
		      <p>{{ @$order->dormitory->address or '' }}</p>
		      <p>+65: {{ $order->phone_no or '' }}</p>
		    @endif
			</div>
		</td>
	</tr>
	...
	<tr>
		@if(@$restaurant->nea_number)
		    <td><b>Invoice Id:</b> {{$order->invoice_id}}</td>
			<td><b>NEA LICENSE:</b> {{ @$restaurant->nea_number }}</td>
		@else
